...
Salvo em: ${dateStr}`;
                    ...
                    // Check Completion!
                    if (checkScheduleCompletion(parsed.selectedClasses)) {
...
function checkScheduleCompletion(validationData = null) {
  ...
  for (const discipline of allDisciplines) {
    ...
    selected.forEach(turmaCode => {
      ...
      if (classInfo) {
        // Use the type we preserved, or infer from code if needed (fallback)
        const type = classInfo.type || (turmaCode.startsWith("TP") ? "TP" : "PL");
        
        if (type === "TP") tpCount++;
        else if (type === "PL") plCount++;
      }
    });

    // Requirement: Exactly 1 TP and 1 PL
    if (tpCount !== 1 || plCount !== 1) {
      return false;
    }
  }
...
function updateCompletionStatus() {
  const isComplete = checkScheduleCompletion();
  const container = document.getElementById("completionStatus");
  
  // Elements to toggle success class
  const csvBtn = document.getElementById("csvBtn");
  ...
  if (isComplete) {
    ...
    // Update CSV Button
    if (csvBtn) csvBtn.classList.add("success");
    
  } else {
    // Show incomplete message
    container.textContent = "(Horário Incompleto)";
    container.className = "completion-message incomplete show"; 
    
    // Revert Button
    if (csvBtn) csvBtn.classList.remove("success");
  }
}
